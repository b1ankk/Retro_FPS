...
namespace game
{
    ...
    void HealthUIE::createTexture(unique_ptr<sf::Texture>& texturePtr) const
    {
        ...
        // renderTexture.clear(sf::Color{0xffffff00}); // White alpha
        //
        // auto left  = Game::get().textureManager()->getTextureForName("base_left");
        // auto fill  = Game::get().textureManager()->getTextureForName("base_strap");
        // auto right = Game::get().textureManager()->getTextureForName("base_right");
        //
        // assert(size().x >= left->getSize().x + right->getSize().x);
        //
        // sf::RectangleShape leftRect{static_cast<sf::Vector2f>(left->getSize())};
